            ...
            VStack(alignment: .leading, spacing: 24) {
                ...
                section("Key Concepts") {
                    ...
                    concept("Sandbox Backends",
                            "Optional isolation for Claude sessions, set globally (Settings), per project, or per session (New Worktree Session sheet). Docker Sandbox runs Claude in an sbx microVM (requires Docker Desktop; no session resume). Apple container runs it in a lightweight VM via Apple's container runtime (macOS 26+, Apple silicon; resume works). Sandboxed sessions show a shield icon in the sidebar — hover it to see which backend. The sandbox protects your machine (home dir, SSH keys, Keychain, other repos stay out of reach) — not the mounted project itself, and outbound network stays open. See the User Guide for the full boundary.")
