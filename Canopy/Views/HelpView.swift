            ...
            VStack(alignment: .leading, spacing: 24) {
                ...
                section("Key Concepts") {
                    ...
                    concept("Sandbox Backends",
                            "Optional isolation for Claude sessions, set globally (Settings), per project, or per session (New Worktree Session sheet). They protect different amounts. Claude sandbox uses Claude Code's own macOS Seatbelt sandbox: nothing to install, resume works, but it confines Bash only and does NOT restrict reads, so credentials stay readable. Docker Sandbox (legacy) runs Claude in an sbx microVM (requires Docker Desktop; no session resume). Apple container runs it in a lightweight VM via Apple's container runtime (macOS 26+, Apple silicon; resume works) and is the strongest. Sandboxed sessions show a shield icon in the sidebar — hover it to see which backend. For the VM backends, everything not explicitly mounted (SSH keys, Keychain, other repos, the rest of your home dir) stays out of reach — but not the mounted project and its main repo, or Claude state, and outbound network stays open. See the User Guide for the full boundary. If a backend isn't ready when a session starts — for example the container runtime was stopped — Canopy prints the fix in the terminal instead of a cryptic error.")
